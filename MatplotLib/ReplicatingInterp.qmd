...
Given these characteristics, we can establish clear selection criteria. For large pre-triangulated meshes from finite element analysis, `tripcolor` is typically the appropriate choice since performance is critical and the individual elements are usually small enough that interpolation artifacts are not visually distracting. Conversely, when generating high-quality figures of a few large polygons for exercises or instructional materials, `tricontourf` is often the superior method. The performance penalty is acceptable in this context, and the resulting visual smoothness more accurately conveys the continuous nature of the underlying data field, free of triangulation artifacts.
...
The `patch` function implements automatic method selection when `FaceColor='interp'` is specified. For small meshes with fewer than 100 elements using non-linear colormaps such as 'jet', 'hsv', or 'hot', the function selects `tricontourf` with 256 levels to achieve high visual fidelity. For large meshes or when using perceptually uniform colormaps such as 'viridis' or 'plasma', the function selects `tripcolor` with Gouraud shading to prioritize performance. Users can override this automatic selection using the `interpolation_method` parameter to force either approach when specific requirements demand explicit control.
...
patch('Faces', connectivity, 'Vertices', P_complex, 'FaceVertexCData', T,
      ...
      'LineWidth', 0.5, ax=ax)
mk.colorbar(ticks=np.arange(20, 41, 2), ax=ax)
ax.set_aspect('equal')
ax.axis('tight')

...
Newton's method, also known as the Newton-Raphson's method ([Joseph Raphson](https://en.wikipedia.org/wiki/Joseph_Raphson)), is a relatively simple, practical, and widely-used solution finding method. The derivative of $f$ is required. This extra information is paid back in speed. It's easy to see that while in some cases the method rapidly converges to a root of the function, in some other cases it may fail to converge at all. This is one reason why it is important not only to understand the construction of the method, but also to understand its limitations.
...
$$
0-f\left(x_0\right)=f^{\prime}\left(x_0\right)\left(x_1-x_0\right) \Rightarrow x_1=x_0-\frac{f\left(x_0\right)}{f^{\prime}\left(x_0\right)} .
$$
...
To illustrate the method, consider finding a root of the curved function $f(x) = x^2 - 2$. The animation below shows the iterative process: starting from an initial guess $x_0$, we draw a vertical line to $f(x_0)$, construct the tangent line at that point, and find where it crosses the x-axis to obtain $x_1$. This process repeats, with each iteration bringing us closer to the root $x^* \approx 2.0946$.

...
1. Run the commands in [Configuring Windows for Development](#configuring-windows) to unlock your machine
2. Use the [Marimo installer](#marimo-installer-windows) to automate the Python setup
3. Read [Working with the Terminal](#working-with-the-terminal) for package management commands

**macOS users:**

1. Read [Terminal Basics](#macos-terminal) if you are new to the command line
2. Follow the [manual installation](#manual-python-installation) to install Python and marimo
3. Set up shortcuts in [Configuring macOS for Development](#macos-config)
:::

## Configuring Windows for Development {#configuring-windows}
...
The `CurrentUser` scope takes precedence over machine-wide settings, so this ensures scripts run regardless of other policies.
...
New-ItemProperty -Path "HKCU:\Software\Microsoft\Windows\CurrentVersion\Policies\Associations" -Name "DefaultFileTypeRisk" -Value 0x1808 -PropertyType DWord -Force 2>$null
```

To unblock files you have already downloaded, navigate to the folder containing them and run:

```powershell
Get-ChildItem -Recurse | Unblock-File
```

### Disable SmartScreen for Development
...
This downloads and installs WSL 2 with Ubuntu. After installation completes, **restart your computer**. On the next boot, Ubuntu will finish setting up and prompt you to create a username and password for the Linux environment.
...
If you did not use the installer, activate manually:
...
Open Ubuntu from the Start menu (or type `wsl` in any terminal). Then run:

```bash
source ~/.venvs/default/bin/activate
```

Your Windows files are at `/mnt/c/Users/yourusername/`. For example, OneDrive is typically at `/mnt/c/Users/yourusername/OneDrive/`.
...
## Terminal Basics (macOS) {#macos-terminal}

macOS includes a powerful terminal that gives you direct control over your system. Unlike Windows, macOS does not fight you at every step. The terminal is ready to use without configuration changes. If you are comfortable with the terminal, skip to [Manual Python Installation](#manual-python-installation).
...
|---------|-------------|---------|
| `pwd` | Print working directory (where you are) | `pwd` → `/Users/anna/Documents` |
| `ls` | List files in current folder | `ls` |
| `ls -alh` | List all files (including hidden) with details | `ls -alh` |
| `cd` | Change directory | `cd Documents` |
| `cd ~` | Go to home folder | `cd ~` |
| `cd ..` | Go up one folder | `cd ..` |
| `mkdir` | Create a new folder | `mkdir projects` |
| `rm` | Remove a file | `rm oldfile.py` |
| `rm -r` | Remove a folder and its contents | `rm -r oldfolder` |
| `cp` | Copy a file | `cp file.py backup.py` |
| `mv` | Move or rename a file | `mv old.py new.py` |
...
The file opens in the terminal. Type or edit text as needed. The bottom of the screen shows available commands, where `^` means the Control key.
...
This creates a folder called `.venvs/default` in your WSL home directory (not on the Windows filesystem). To activate the virtual environment:
...
Your Python files can live on OneDrive (at `/mnt/c/Users/yourusername/OneDrive/`) while the virtual environment stays in the fast WSL filesystem.
...
```bash
alias activate="source ~/.venvs/default/bin/activate"
```

Save and exit (`Ctrl+X`, `Y`, `Enter`), then reload the configuration:
...
```powershell
Add-Content $PROFILE 'function startMarimo { & "$env:USERPROFILE\.venvs\default\Scripts\Activate.ps1"; Write-Host "venv: $env:VIRTUAL_ENV"; marimo --version; marimo edit }'
```
...
Now you can start marimo from any folder, just navigate there in explorer and right-click → "Open in Terminal", then run:
...
Now you can start marimo from any folder:
...
Marimo opens in your browser with access to files in the current folder. The terminal shows the server running. Press `Ctrl+C` to stop marimo when done.
...
## Configuring macOS for Development {#macos-config}
...
### Where to Save Your Python Files
...
Or if OneDrive uses a different folder name:

```bash
cd ~/Library/CloudStorage/OneDrive-Personal
```

You can find the exact path by dragging your OneDrive folder from Finder into the terminal window.
...
You skipped the [Configuring Windows for Development](#configuring-windows) section. Go back and run those commands, then restart your terminal.

...
Download the installer from [here](https://github.com/cenmir/marimo-installer/archive/refs/heads/main.zip).
...
The installer adds an "Activate venv" shortcut. You can also activate the environment manually in PowerShell:
...
See [this section](ProgrammingFundamentals/writing_documentation.ipynb) for quarto installation and detailed usage.

Quarto converts Jupyter notebooks to PDF, HTML, or other formats. With the environment active, you can render a notebook:
...
Quarto requires a LaTeX installation for PDF output. If you don't have LaTeX installed, quarto can install TinyTeX for you:
...
| Task | Command |
|------|---------|
| Activate environment | `. $env:USERPROFILE\venvs\default\Scripts\Activate.ps1` |
| Install a package | `uv pip install package_name` |
| Upgrade a package | `uv pip install --upgrade package_name` |
| List installed packages | `uv pip list` |
| Render notebook to PDF | `quarto render notebook.ipynb --to pdf` |
| Render notebook to HTML | `quarto render notebook.ipynb --to html` |

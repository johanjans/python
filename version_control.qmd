...
Git is a version control system that helps you track changes in your code locally on your machine. This guide covers the essential commands you need to manage your projects offline with Git on Windows.

## Prerequisites

Before starting, make sure you have Git installed on your Windows machine. Download it from [git-scm.com](https://git-scm.com/download/win) if needed.
...
1. **Use Git Bash** for the most consistent experience across platforms
2. **File paths** use forward slashes (`/`) in Git commands, even on Windows
3. **Line endings**: Git handles Windows line endings (CRLF) automatically
4. **.gitignore**: Use `.gitignore` to exclude files you don't want to track (e.g., `*.tmp`, `*.log`)

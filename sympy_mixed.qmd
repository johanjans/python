...
```{python}
# Moment M = r x F
r = sp.Matrix([1, 0, 0]) # Position vector
M = r.cross(F_A)
M
```
...
```{python}
A_vec = 20 * sp.Matrix([sp.cos(sp.pi/2), sp.sin(sp.pi/2), 0])
B_vec = sp.Matrix([sp.sqrt(2), sp.sqrt(3), 0])

# Vector projection of A along B
A_proj_B = A_vec.project(B_vec)
A_proj_B
```
...
# Lambdify
W_func = sp.lambdify([m, g], W, 'numpy')

# Evaluate and flatten
W_np = W_func(20, 9.81).flatten()
W_np
```
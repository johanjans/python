...
```{python}
#| echo: false
Markdown(f"$\\frac{{d}}{{dx}} {sp.latex(f)}$ = ${sp.latex(df_dx)}$")
```
...
```{python}
#| echo: false
Markdown(f"$\\int {sp.latex(sp.cos(x))}$ = ${sp.latex(integral)}$")
```
...
Integrate $\int_0^1 \int_0^1 x y \, dx \, dy$.

```{python}
# Double integral of f(x, y) = x*y over the unit square
x, y = sp.symbols('x y')
f = x * y
